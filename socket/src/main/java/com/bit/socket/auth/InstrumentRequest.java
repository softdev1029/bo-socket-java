...
public class InstrumentRequest extends Message {
    ...
    public void makeByteBuffer() {
        ...
        byteBuffer.put(Data1).put(Data2).putShort(MessageLen).putShort(MessageType).putInt(RejectReason4Byte)
                .putInt(Account).putShort(RequestType).putInt(Key).put(SymbolName).position(44).putShort(SymbolType)
                .putShort(SymbolEnum).putInt(TradingSessionID).putLong(SendingTime).putInt(MsgSeqNum);
    }
