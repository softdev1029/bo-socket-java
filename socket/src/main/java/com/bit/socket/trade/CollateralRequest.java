...
public class CollateralRequest extends Message {
    ...
    public void makeByteBuffer() {
        ...
        byteBuffer.put(Data1).put(Data2).putShort(MessageLen).putShort(MessageType).putShort(UpdateType).putInt(Account)
                .putInt(TradingSessionID).putShort(SymbolEnum).putInt(Key).putInt(MsgSeqNum).putLong(SendingTime);
    }
