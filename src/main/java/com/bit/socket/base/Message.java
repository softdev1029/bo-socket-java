...
abstract public class Message {
    ...
    public void send(DataOutputStream out) {
        try {
            ...
            Logger.logf("Sending %s message, %s bytes ...", MessageTypeStr, MessageLen);
    ...
    public int read(DataInputStream in) {
        try {
            ...
            Logger.logf("Received %s message, need %s bytes, read %s bytes.", MessageTypeStr, MessageLen,
    ...
    protected boolean validate() {
        if (MessageType < Constant.MSGTYPE_MIN_VALUE || MessageType > Constant.ORDTYPE_MAX_VALUE) {
            RejectReason = RejectCode.MSG_TYPE_INVALID;
            return false;
        }

        if (OrderType < Constant.ORDTYPE_MIN_VALUE || OrderType > Constant.ORDTYPE_MAX_VALUE) {
            RejectReason = RejectCode.ORD_TYPE_INVALID;
            return false;
        }

        if (MessageType == MessageTypes.ORDER_NEW) {
            if (Util.shortArrayContains(new short[] { OrderTypes.LMT, OrderTypes.HIDDEN, OrderTypes.PEG,
                    OrderTypes.PEG_HIDDEN, OrderTypes.OCO, OrderTypes.ICE, }, OrderType)) {
                byte ds = Attributes[Attribute.DISPLAYSIZE_ATTRIBUTE];
                if (ds == 'Y') {
                    if (DisplaySize <= 0) {
            ...
            } else if (Util.shortArrayContains(new short[] { OrderTypes.STOP_MKT, OrderTypes.STOP_LMT,
                    OrderTypes.SNIPER_MKT, OrderTypes.SNIPER_LIMIT, OrderTypes.TSM, OrderTypes.TSL, }, OrderType)) {
                short se = SymbolEnum;
                if (se < Constant.SE_MIN_VALUE || se > Constant.SE_MAX_VALUE) {
                    RejectReason = RejectCode.SYMBOL_ENUM_INVALID;
                    return false;
    ...
    public void printRejectReason() {
        ...
        } else if ((RejectReason == RejectCode.ICE_SIZEINCREMENT_TIMES_LAYERS_NOT_EQUAL_ORDQTY)) {
        ...
        } else if ((RejectReason == RejectCode.PERCENTAGE_MOVE_EXCEEDED_COOLING_OFF_PERIOD_IN_FORCE)) {
            msg += "PERCENTAGE_MOVE_EXCEEDED_COOLING_OFF_PERIOD_IN_FORCE";
        } else if ((RejectReason == RejectCode.INSTRUMET_WOULD_CAUSE_MARGIN_TO_BE_EXCEEDED)) {
        ...
        } else if ((RejectReason == RejectCode.MARGIN_SELL_ORDER_CANCELLATION_IN_PROGRESS)) {
            msg += "MARGIN_SELL_ORDER_CANCELLATION_IN_PROGRESS";
        } else if ((RejectReason == RejectCode.MARGIN_LONG_POSITION_LIQUIDATION_IN_PROGRESS)) {
            msg += "MARGIN_LONG_POSITION_LIQUIDATION_IN_PROGRESS";
        } else if ((RejectReason == RejectCode.MARGIN_SHORT_POSITION_LIQUIDATION_IN_PROGRESS)) {
